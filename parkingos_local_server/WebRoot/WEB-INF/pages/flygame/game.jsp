...
<script src="js/flygame.2.1.min.js?a=5555" type="text/javascript">//打灰机核心js</script>
<style type="text/css">
		html,body {
		    padding: 0 !important;
			margin: 0 !important;
...
function getScroe(type,bnums){//bnums 剩余子弹数量
	...
	var data =false;
	try{
		data=eval(T.A.sendData("flygame.do","POST","action=getscore&uin=${uin}&tid=${tid}&type="+type+"&bnums="+bnums));
	}catch(e){
		resumeGame();
	}
	if(!data){
	...
	if(addBtn=='toshare'){
		if(btype!='ios'&&((type=='plane_ticket'&&data[0].money>3)||(type=='plane_cash'&&data[0].money>=1)))
			...
			sharetitle.innerHTML='关注公众号，可用券抵扣停车费';
		...
		sharetitle.innerHTML='进入页面后，电话咨询，了解详情';
	}else if(addBtn=='1818'){
	...
	if(btype=='ios'){//ios客户端不分享和跳转
		...
		if(addBtn=='toshare'||addBtn=='fudai'){
			...
			sharespan.innerHTML='您得到了一张神秘礼券，可以我的战绩中查看';
		}else if(addBtn=='1818'){
			sharespan.innerHTML='您得到了一张投资礼券，可以我的战绩中查看';
		}else if(addBtn=='topublic'){
...
function shareinfo(bnums,sid,money,type){//bnums 剩余子弹数量
	//type=='plane_ticket'||type=='plane_cash'
	if(type=='plane_ticket'&&money<4)
		return ;
	if(type=='plane_cash'&&money<1)
		return ;
	var btype = getClientType();
	var sourcetag = navigator.userAgent;
	if(btype=='wx'){
		location = "flygame.do?action=toshare&uin=${uin}&page=play&tid=${tid}&bnums="+bnums+"&sid="+sid;
	}else{

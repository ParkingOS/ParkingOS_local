	...
	<div id="scroller">
		<ul id="thelist">
			...
			<ul class="hide">
				...
				<input type="text" name="notice_type" value="5">
				<input type="text" name="openid" value="${openid}">
				<input type="text" name="ticketmoney" value="${ticketmoney}">
				<input type="text" name="ticketnum" value="${ticketnum}">
			</ul>
	...
	function shade(){
	 ...
	 function timer(){
		...
		$(".DialogDiv").css("display", "block");
	 	setTimeout("buyticket()",2000);//两秒后执行
	 }
 </script>
<script type="text/javascript">
		 ...
		 WeixinJSBridge.invoke('getBrandWCPayRequest',{  
             ...
             "paySign" : '${paySign}'           //微信签名  
            	 ...
            	 if(res.err_msg == "get_brand_wcpay_request:ok"){
            		 window.location.href = "wxpublic.do?action=balancepayinfo&openid=${openid}&money=${total}&ticketmoney=${ticketmoney}&ticketnum=${ticketnum}&notice_type=5";
            	 }
         });
	}
	
	var index = 0;//防止订单重复提交
	function buyticket(){//不调起微信支付
		jQuery.ajax({
			type : "post",
			url : "carowner.do",
			data : {
